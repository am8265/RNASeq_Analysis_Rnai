...
hmcol = colorRampPalette(c("red","yellow","green"))(n=299)
###plotting commonly DGE based on "AM"...Top genes with high average expression across all samples 
...
fpkm_counts.intsct.AM.u <- fpkm_counts.intsct.AM [order(-rowMeans(fpkm_counts.intsct.AM)),][readFile(opt$geneIDs),]

top70.intsct.AM <- rownames(fpkm_counts.intsct.AM)#just the gene ids

top70.intsct.AM.genes <- fpkm_counts_dummy[top70.intsct.AM, 16]# The ordered geneNames
top70.intsct.AM.names.row <- sub('^NA:', '', x = paste(top70.intsct.AM.genes, top70.intsct.AM, sep = ":"))
heatmap.2(fpkm_counts.intsct.AM, labRow = top70.intsct.AM.names.row, offsetRow = -0.6, 
          col = hmcol, trace="none", margin=c(6, 20),srtCol=30, offsetCol =-1.5,cexCol=1.0,ColSideColors = c(rep("gray",3),rep("blue",3),rep("black",2),rep("orange",2),rep("darkgreen",3),rep("navyblue",2)))
par(lend=1)
legend(x=0.78,y=1, legend=c("X717c.Control_A_Shoot","X717c.Control.A_Axillary.bud","IAA7.52_shoot.tip","IAA7.52_axillary.bud","IAA7.54_shoot.tip","IAA7.54_axillary.bud"), col = c("gray","blue","black","orange","darkgreen","navyblue"), lty=1:2,lwd=10, box.lty = 0,cex=0.5)
dev.off()

fpkm_counts.intsct.UK <- fpkm_counts.intsct.UK [order(-rowMeans(fpkm_counts.intsct.UK)),][c(1:70),]
top70.intsct.UK <- rownames(fpkm_counts.intsct.UK)
top70.intsct.UK.genes <- fpkm_counts_dummy[top70.intsct.UK, 16]
top70.intsct.UK.names.row <- sub('^NA:', '', x = paste(top70.intsct.UK.genes, top70.intsct.UK, sep = ":"))
png("fpkm_counts.intsct.UK.png", height = 5*300, width = 5*300, res=300, pointsize = 8)
heatmap.2(fpkm_counts.intsct.UK, labRow = top70.intsct.UK.names.row, offsetRow = -0.6, 
          col = hmcol, trace="none", margin=c(6, 20),srtCol=30, offsetCol =-1.5,cexCol=1.0,ColSideColors = c(rep("gray",3),rep("blue",3),rep("black",2),rep("orange",2),rep("darkgreen",3),rep("navyblue",2)))
par(lend = 1)
legend(x=0.78,y=1, legend=c("X717c.Control_A_Shoot","X717c.Control.A_Axillary.bud","IAA7.52_shoot.tip","IAA7.52_axillary.bud","IAA7.54_shoot.tip","IAA7.54_axillary.bud"), col = c("gray","blue","black","orange","darkgreen","navyblue"), lty=1:2,lwd=10, box.lty = 0,cex=0.5)
dev.off()
...
top70.union.UK <-  rownames(fpkm_counts.union.UK)
top70.union.UK.genes <- fpkm_counts_dummy[top70.union.UK, 16]
top70.union.UK.names.row <- sub('^NA:', '', x = paste(top70.union.UK.genes, top70.union.UK, sep = ":"))
png("fpkm_counts.union.UK.png",height = 5*300, width = 5*300, res=300, pointsize = 8)
heatmap.2(fpkm_counts.union.UK, labRow = top70.union.UK.names.row, offsetRow = -0.6, 
          col = hmcol, trace="none", margin=c(6, 20),srtCol=30, offsetCol =-1.5,cexCol=1.0,ColSideColors = c(rep("gray",3),rep("blue",3),rep("black",2),rep("orange",2),rep("darkgreen",3),rep("navyblue",2)))
par(lend=1)
legend(x=0.78,y=1, legend=c("X717c.Control_A_Shoot","X717c.Control.A_Axillary.bud","IAA7.52_shoot.tip","IAA7.52_axillary.bud","IAA7.54_shoot.tip","IAA7.54_axillary.bud"), col = c("gray","blue","black","orange","darkgreen","navyblue"), lty=1:2,lwd=10, box.lty = 0,cex=0.5)
dev.off()

fpkm_counts.union.AM <- fpkm_counts.union.AM[order(-rowMeans(fpkm_counts.union.AM)),][c(1:70),]
top70.union.AM <- rownames(fpkm_counts.union.AM)
top70.union.AM.genes <- fpkm_counts_dummy[top70.union.AM, 16]
top70.union.AM.row.names <- sub('^NA:', '', x = paste(top70.union.AM.genes, top70.union.AM, sep = ":"))
png("fpkm_counts.union.AM.png",height = 5*300, width = 5*300, res=300, pointsize = 8)
heatmap.2(fpkm_counts.union.AM, labRow = top70.union.AM.row.names, offsetRow = -0.6, 
          col = hmcol, trace="none", margin=c(6, 20),srtCol=30, offsetCol =-1.5,cexCol=1.0,ColSideColors = c(rep("gray",3),rep("blue",3),rep("black",2),rep("orange",2),rep("darkgreen",3),rep("navyblue",2)))
par(lend=1)
legend(x=0.78,y=1, legend=c("X717c.Control_A_Shoot","X717c.Control.A_Axillary.bud","IAA7.52_shoot.tip","IAA7.52_axillary.bud","IAA7.54_shoot.tip","IAA7.54_axillary.bud"), col = c("gray","blue","black","orange","darkgreen","navyblue"), lty=1:2,lwd=10, box.lty = 0,cex=0.5)
dev.off()
